...
void PowerIteration(mat A, double &eiganval, mat &eiganvector);
	...
	mat::mat(double t, int cols, int rows) {
	...
	mat::mat(vector<vector<double>> d) {
		mat t(0, d.size(), d[0].size());
		t.data = d;
		data = t.transpose().data;
	}
	double mat::operator()(int i = 0) {
		return data[i][i];
	}
	vector<double>& mat::operator[](int i) {
		return data[i];
	}
	mat mat::col(unsigned int ind) {
		mat out(0, 1, rows());
		out.data[0] = data[ind];
		return out;
	}
	mat mat::row(unsigned int ind) {
		mat out(0, cols(), 1);
	...
	void mat::insertROW(int ind, mat r) {
		for (int i = 0; i < cols(); i++)
			data[i][ind] = r.data[i][0];
	}
	void mat::insertCOL(int ind, mat r) {
		data[ind] = r.data[0];
	}
	int mat::cols() {
		return data.size();
